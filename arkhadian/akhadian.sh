...
echo " ██║ ╚████║╚██████╔╝██████╔╝███████╗██╔╝ ██╗    ╚██████╗██║  ██║██║     ██║   ██║   ██║  ██║███████╗";
echo " ╚═╝  ╚═══╝ ╚═════╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝     ╚═════╝╚═╝  ╚═╝╚═╝     ╚═╝   ╚═╝   ╚═╝  ╚═╝╚══════╝";
echo ">>> Cosmovisor Automatic Installer for Arkhadian | Chain ID : arkh <<<";
echo -e "\e[0m"
...
GENESIS=https://mainnet-files.itrocket.net/arkhadian/genesis.json
ADDRBOOK=https://mainnet-files.itrocket.net/arkhadian/addrbook.json
PORT=125
...
curl -Ls https://go.dev/dl/go1.19.3.linux-amd64.tar.gz | sudo tar -xzf - -C /usr/local
...
mv bin/$BINARY $HOME/$FOLDER/$COSMOVISOR/genesis/bin/$BINARY
rm -rf build
...
# Set peers and
PEERS="$(curl -sS https://arkhadian-mainnet-rpc.itrocket.net/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"
SEEDS="8d08350798563bb132db92bead3e8d4cf894a8bb@65.108.70.119:31656"
...
sed -i.bak -e "s%^address = \"tcp://localhost:1317\"%address = \"tcp://127.0.0.1:${PORT}17\"%; s%^address = \":8080\"%address = \"127.0.0.1:${PORT}80\"%; s%^address = \"localhost:9090\"%address = \"127.0.0.1:${PORT}90\"%; s%^address = \"localhost:9091\"%address = \"127.0.0.1:${PORT}91\"%" $HOME/$FOLDER/config/app.toml
